...
export class CvrpEngine {
  ...
  private assignVehicles(
    ...
    if (availableVehicles.length === 0) return; // Guard clause

    for (const route of routes) {
      // Find the next available driver matching the assigned shift time
      let availableDriver = availableVehicles.find(v => v.shift === route.shiftSlot);
      
      // If no explicit driver matches this shift, fallback to any active real driver
      if (!availableDriver) {
        availableDriver = availableVehicles[0];
      }
